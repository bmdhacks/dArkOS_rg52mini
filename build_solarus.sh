...
if [ -f "Arkbuild_package_cache/${CHIPSET}/solarus.tar.gz" ] && [ "$(cat Arkbuild_package_cache/${CHIPSET}/solarus.commit)" == "$(curl -s https://raw.githubusercontent.com/christianhaitian/${CORE_BUILDS_CHIPSET}_core_builds/refs/heads/master/scripts/solarus.sh | grep -oP '(?<=TAG=").*?(?=")')" ]; then
	...
	sudo curl -s https://raw.githubusercontent.com/christianhaitian/${CORE_BUILDS_CHIPSET}_core_builds/refs/heads/master/scripts/solarus.sh | grep -oP '(?<=TAG=").*?(?=")' > Arkbuild_package_cache/${CHIPSET}/solarus.commit
fi
