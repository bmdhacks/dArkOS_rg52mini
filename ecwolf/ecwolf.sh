...
then
  ...
  cd /$directory/wolf
  ./Scan_for_new_games.wolf
  printf "\n\nFinished scanning the wolf folder for games." >> /dev/tty1
...
then
  ...
  /usr/local/bin/retroarch -L /home/ark/.config/retroarch/cores/ecwolf_libretro.so "$game"
fi
